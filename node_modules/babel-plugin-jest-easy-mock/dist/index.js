...
function plugin({ types: t, template: tmpl }, options) {
    let config = Object.assign(Object.assign({}, defaultConfig), options);
    /**
     * Return full name of member expression
     */
    const getNameOfExpression = (node, name = "") => {
     ...
     */
    const isJestMockCallExpression = (node) => {
     ...
     */
    const isMockObjCallExpession = (node) => {
        if (!t.isIdentifier(node.callee) && !t.isMemberExpression(node.callee)) {
            return undefined;
        }
        let fullCalle = getNameOfExpression(node.callee);
    ...
    const buildArrowFunc = (statements, modulePath) => tmpl(`
        () => {
    ...
    `, { placeholderPattern: false, placeholderWhitelist: new Set(["OBJECT_EXP"]) });
    const buildNameStatement = (key, name) => `Object.defineProperty(o.${key}, "name", { value: "${name}" });`;
    /**
     * Create module mock implementation
     */
    const createModuleMockImpl = (path, def) => {
        const topLevelProps = [];
        const statements = [];
                ...
                for (const m of mocks) {
                    if (!m.subProperty) {
                        ...
                        if (config.requireActual && prop.value.properties.length === 0) {
                            prop.value.properties.push(t.spreadElement(t.memberExpression(t.identifier("a"), t.identifier(key))));
                        }
        ...
        const objExp = t.objectExpression(config.requireActual ? [t.spreadElement(t.identifier("a")), ...topLevelProps] : topLevelProps);
        const nameStatements = statements
            ...
            const oKey = prop.value.properties.find(p => t.isObjectProperty(p) &&
        ...
        const arrowFunc = buildArrowFunc(nameStatements, config.requireActual ? path : undefined)({ OBJECT_EXP: objExp });
        return arrowFunc.expression;
    };
    /**
     * Build jest.mock() expression
     */
    const buildJestMockForModule = (source, mocks) => {
        return t.expressionStatement(t.callExpression(t.memberExpression(t.identifier(config.jestIdentifier), t.identifier("mock")), [
            ...
            this.createMockDefinition = (def, type, name, impl) => {
                ...
                if (!impl) {
                    ...
                    impl = type === "name"
                        ...
                        : t.callExpression(t.memberExpression(t.callExpression(t.memberExpression(t.identifier("jest"), t.identifier("fn")), []), t.identifier("mockName")), [t.stringLiteral(name)]);
                }
